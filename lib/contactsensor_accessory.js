  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (var statusMap of statusArr) {
      if (statusMap.code === 'doorcontact_state') {
        ...
        this.log.log(`Contact Value: ${this.sensorStatus.value} = ${statusMap.value}`);
        const hbSensorState = this.tuyaParamToHomeBridge(Characteristic.ContactSensorState, this.sensorStatus.value);
        this.log.log(`Contact State 2: ${this.sensorStatus.value} = ${hbSensorState}`);
        this.normalAsync(Characteristic.ContactSensorState, 1);
      }
