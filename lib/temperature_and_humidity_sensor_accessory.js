    ...
    refreshAccessoryServiceIfNeed(stateArr, isRefresh) {
        ...
        for (const statusMap of stateArr) {
            if (statusMap.code === 'va_temperature') {
                let temperature = statusMap.value / 10;
this.log.log(`temperatureAccessory temp ${temperature}`);
...
this.log.log(`temperatureAccessory humidity ${statusMap.value}`);
                this.normalAsync(this.humidityService, Characteristic.CurrentRelativeHumidity, statusMap.value);
            }
        }
    }
